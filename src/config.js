...
export function loadConfig(env = process.env, { requireTelegram = true } = {}) {
  ...
  return {
    telegramBotToken,
    telegramChatId,
    checkIntervalSeconds: integer(env, 'CHECK_INTERVAL_SECONDS', 120, { min: 60, max: 3600 }),
    confirmationsRequired: integer(env, 'CONFIRMATIONS_REQUIRED', 1, { min: 1, max: 10 }),
    missingChecksBeforePrune: integer(env, 'MISSING_CHECKS_BEFORE_PRUNE', 3, { min: 2, max: 1440 }),
    ...
    requestWindowSeconds: integer(env, 'REQUEST_WINDOW_SECONDS', 60, { min: 20, max: 300 }),
    minRequestGapSeconds: integer(env, 'MIN_REQUEST_GAP_SECONDS', 2, { min: 2, max: 30 }),
